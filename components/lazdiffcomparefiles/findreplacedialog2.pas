 ...
 *****************************************************************************
  ...
  Abstract:
    ...
    Usage:
      Add to program
        "Application.CreateForm(TLazFindReplaceDialog2, FindReplaceDlg);"
...
type
  ...
  { TLazFindReplaceDialog2 }

  TLazFindReplaceDialog2 = class(TForm)
...
var
  LazFindReplaceDialog2: TLazFindReplaceDialog2 = nil;
...
constructor TLazFindReplaceDialog2.Create(TheOwner:TComponent);
begin
  inherited Create(TheOwner);
  Caption:='';
  TextToFindComboBox.Text:='';
  TextToFindLabel.Caption:=dlgTextToFind2;
  ReplaceTextComboBox.Text:='';
  ReplaceWithCheckbox.Caption:=dlgReplaceWith2;
  IDEImages.AssignImage(EnableAutoCompleteSpeedButton, 'autocomplete');
  OptionsGroupBox.Caption:=lisOptions2;

  with CaseSensitiveCheckBox do begin
    Caption:=dlgCaseSensitive2;
    Hint:=lisDistinguishBigAndSmallLettersEGAAndA2;
  end;

  with WholeWordsOnlyCheckBox do begin
    Caption:=dlgWholeWordsOnly2;
    Hint:=lisOnlySearchForWholeWords2;
  end;

  with RegularExpressionsCheckBox do begin
    Caption:=dlgRegularExpressions2;
    Hint:=lisActivateRegularExpressionSyntaxForTextAndReplaceme2;
  end;

  with MultiLineCheckBox do begin
    Caption:=lisFindFileMultiLinePattern2;
    Hint:=lisAllowSearchingForMultipleLines2;
  end;

  with PromptOnReplaceCheckBox do begin
    Caption:=dlgPromptOnReplace2;
    Hint:=lisAskBeforeReplacingEachFoundText2;
  end;

  OriginGroupBox.Caption := dlgSROrigin2;
  FromCursorRadioButton.Caption := dlgFromCursor2;
  EntireScopeRadioButton.Caption := dlgFromBeginning2;

  ScopeGroupBox.Caption := dlgSearchScope2;
  GlobalRadioButton.Caption := dlgGlobal2;
  SelectedRadioButton.Caption := dlgSelectedText2;

  DirectionGroupBox.Caption := dlgDirection2;
  ForwardRadioButton.Caption := lisFRForwardSearch2;
  BackwardRadioButton.Caption := lisFRBackwardSearch2;

  // CloseButton works now as ReplaceAllButton
  BtnPanel.CloseButton.Caption := dlgReplaceAll2;
...
destructor TLazFindReplaceDialog2.Destroy;
begin
  RegExpr.Free;
  inherited Destroy;
  if LazFindReplaceDialog2=Self then
    LazFindReplaceDialog2:=nil;
end;

procedure TLazFindReplaceDialog2.FormClose(Sender: TObject;
  var CloseAction: TCloseAction);
begin
  IDEDialogLayoutList.SaveLayout(Self);
end;

procedure TLazFindReplaceDialog2.FormShow(Sender: TObject);
begin
...
procedure TLazFindReplaceDialog2.UpdateHints;
begin
  if EnableAutoComplete then
    EnableAutoCompleteSpeedButton.Hint:=lisAutoCompletionOn2
  else
    EnableAutoCompleteSpeedButton.Hint:=lisAutoCompletionOff2;
end;

procedure TLazFindReplaceDialog2.ResetUserHistory;
var
  c: TFindDlgComponent;
begin
...
procedure TLazFindReplaceDialog2.RestoreState(const AState: TLazFindReplaceState);
begin
...
procedure TLazFindReplaceDialog2.SaveState(out AState: TLazFindReplaceState);
begin
...
procedure TLazFindReplaceDialog2.TextToFindComboboxKeyDown(Sender: TObject;
...
procedure TLazFindReplaceDialog2.HelpButtonClick(Sender: TObject);
begin
  LazarusHelp.ShowHelpForIDEControl(Self);
end;

procedure TLazFindReplaceDialog2.OptionsGroupBoxResize(Sender: TObject);
var
  h: integer;
begin
  DisableAlign;
...
procedure TLazFindReplaceDialog2.ReplaceWithCheckboxChange(Sender: TObject);
begin
  ...
  begin
    Caption := lisReplace2;
    BtnPanel.OKButton.Caption := lisBtnReplace2;
  end else
  begin
    Caption := lisMenuFind2;
    BtnPanel.OKButton.Caption := lisBtnFind2;
  end;
end;

procedure TLazFindReplaceDialog2.FormChangeBounds(Sender: TObject);
var
  w: integer;
begin
  DisableAlign;
...
procedure TLazFindReplaceDialog2.EnableAutoCompleteSpeedButtonClick(
  Sender: TObject);
begin
...
procedure TLazFindReplaceDialog2.OkButtonClick(Sender:TObject);
begin
...
procedure TLazFindReplaceDialog2.ReplaceAllButtonClick(Sender:TObject);
begin
  if not CheckInput then exit;
...
procedure TLazFindReplaceDialog2.CancelButtonClick(Sender:TObject);
begin
  ActiveControl:=TextToFindComboBox;
end;

function TLazFindReplaceDialog2.CheckInput: boolean;
begin
  Result:=false;
  if RegularExpressionsCheckBox.Checked then begin
    ...
    try
      ...
      on E: ERegExpr do begin
        IDEMessageDialog(lisUEErrorInRegularExpression2,
...
function TLazFindReplaceDialog2.GetComponentText(c: TFindDlgComponent): string;
begin
  case c of
...
function TLazFindReplaceDialog2.GetEnableAutoComplete: boolean;
begin
  Result:=EnableAutoCompleteSpeedButton.Down;
end;

procedure TLazFindReplaceDialog2.SetComponentText(c: TFindDlgComponent;
  const AValue: string);
begin
  case c of
...
procedure TLazFindReplaceDialog2.SetEnableAutoComplete(const AValue: boolean);
begin
...
procedure TLazFindReplaceDialog2.SetOnKey(const AValue: TOnFindDlgKey);
begin
  FOnKey:=AValue;
end;

procedure TLazFindReplaceDialog2.SetOptions(NewOptions:TSynSearchOptions);
begin
...
function TLazFindReplaceDialog2.GetOptions:TSynSearchOptions;
begin
  Result:=[];
...
function TLazFindReplaceDialog2.GetFindText:AnsiString;
begin
  Result:=TextToFindComboBox.Text;
end;

procedure TLazFindReplaceDialog2.SetFindText(const NewFindText: AnsiString);
begin
...
function TLazFindReplaceDialog2.GetReplaceText:AnsiString;
begin
  Result:=ReplaceTextComboBox.Text;
end;

procedure TLazFindReplaceDialog2.SetReplaceText(const NewReplaceText:AnsiString);
begin
  SetComboBoxText(ReplaceTextComboBox,NewReplaceText);
end;

procedure TLazFindReplaceDialog2.SetComboBoxText(AComboBox:TComboBox;
